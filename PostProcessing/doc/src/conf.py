...
class LumberjackOutputStructureLexer(RegexLexer):
    name = "text_with_comments"

    tokens = {
        'root': [
            (r'#.*$', token.Comment),
            (r'[┆└├─│]+', token.Operator),
            (r'\(root directory\)', token.Operator),
            (r'([^ ]+)(/)', bygroups(token.Keyword, token.Text)),
            (r'[ph]([23]d)?_[^ ]+', token.Name.Namespace),
            (r'.', token.Text),
        ]
    }
...
sys.modules['Lumberjack'] = Lumberjack
...
project = u'Karma'
copyright = u'2019, Daniel Savoiu'
...
try:
    ...
    release = "dev"  # the full version, including alpha/beta/rc tags.
else:
...
#html_logo = "_static/img/lumberjack_logo.svg"
#html_logo = "_static/img/palisade_logo.svg"
