...
if __name__ == "__main__":
    ...
    _dirs = list(_k.GetName() for _k in _f.GetListOfKeys())
    _trees = set(_k.GetName() for _k in _f.Get(_dirs[0]).GetListOfKeys())
    _f.Close()
    ...
    for _dir in _dirs:
        _dir_obj = _f_out.mkdir(_dir)
        _dir_obj.cd()
        for _tree in _trees:
            _tchain = ROOT.TChain(_tree)
            _full_tree_path = "{}/{}".format(_dir, _tree)
